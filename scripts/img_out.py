...
class CameraManipulator:
    def __init__(self, in_dim, border, crop, out_dim): #crop = None if no crop
        ...
        # run simultaneously.
        rospy.init_node('resizer', anonymous=True)
        self.in_dim = in_dim
        self.border = border
        ...
        self.pub_color_img = rospy.Publisher("manipulate_camera/color/image_raw",
                                             Image, queue_size=10)
	self.pub_color_info = rospy.Publisher("manipulate_camera/color/camera_info",
                                              CameraInfo, queue_size=10)
        self.pub_depth_img = rospy.Publisher("manipulate_camera/depth/image_raw",
                                             Image, queue_size=10)
	self.camera_info_buffer = None
        rospy.spin()

    def manipulate_color_img(self, data):
